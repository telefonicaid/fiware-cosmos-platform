...
class CosmosApiAuthenticationServiceTest extends FlatSpec with MustMatchers with FutureMatchers {
  ...
  "Cosmos API authorization" must "retrieve a profile given user credentials" in new Fixture {
    ...
    cosmosApi.withServer {
      client.authenticate(origin, userCredentials) must eventually(be(profile))
    }
  }

  it must "fail if authentication service is not available" in new Fixture {
    val credentials = UserCredentials("key", "secret")
    client.authenticate(origin, credentials) must
  ...
  it must "fail if profile is malformed" in new Fixture {
    ...
    cosmosApi.withServer {
      client.authenticate(origin, userCredentials) must eventuallyFailWith("Cannot parse profile")
    }
  }
  ...
  it must "fail if infinity secret is misconfigured" in new Fixture {
    ...
    cosmosApi.withServer {
      client.authenticate(origin, userCredentials) must eventuallyFailWith(
        "Authentication was rejected")
    }
  }

  it must "fail if the user profile is accessed from out of the whitelist" in new Fixture {
    val clusterCredentials = ClusterCredentials(clusterSecret = "secret")
    val badOrigin = "10.0.0.2"
    val profile = UserProfile(
      username = "john",
      groups = Seq("scientists"),
      sharedCluster = true,
    ...
    cosmosApi.withServer {
      client.authenticate(origin, clusterCredentials) must eventually(be(profile))
      client.authenticate(badOrigin, clusterCredentials) must eventuallyFailWith("Invalid access")
    }
  }
