  ...
  while (true) {
    ...
    for (int s = 0; s < data->operations_size(); s++) {
      ...
      if( !global_info )
      {
        ...
        global_info = new StreamOperationGlobalInfo(samson_worker_, stream_operation_id, stream_operation_name , worker_ranges);
      ...
      for (size_t r = 0; r < ranges.size(); r++) {
        ...
        if (!stream_operation_info) {
          stream_operation_info = new StreamOperationRangeInfo(samson_worker_
                                                          , stream_operation_id
        ...
        else if (keys_ranges_execution.find(key) == keys_ranges_execution.end()) {  // If not considerd before
      ...
      {
        // Defrag is added without prioritzation
...
gpb::Collection *WorkerTaskManager::getCollectionForStreamOperationsRanges(const ::samson::Visualization& visualization)
{
  // Template-based creation of collection based on map
  return getCollectionForMap("stream operations_ranges"
                             , stream_operations_info_
                             , visualization);
}

  gpb::Collection *WorkerTaskManager::getCollectionForStreamOperations(const ::samson::Visualization& visualization)
  {
    // Template-based creation of collection based on map
    return getCollectionForMap("stream operations"
                               , stream_operations_globla_info_
                               , visualization);
  }
