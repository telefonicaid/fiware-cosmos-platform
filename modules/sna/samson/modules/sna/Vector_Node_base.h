...
namespace sna{
	...
	static inline int size(char *data){
		...
		{ //Getting size of vector items
			...
			for (int i = 0 ; i < (int)_length ; i++){
				offset += ::ss::sna::Node::size(data+offset);
			}
		}
		return offset;
	}
		...
		{ // Comparing vector items
			...
			*offset2 += ss::staticVarIntParse( data2+(*offset2) , &_length2 );
			...
			for (int i = 0 ; i < (int)_length1 ; i++){
				{ // comparing items[i]
					int tmp = ::ss::sna::Node::compare(data1,data2,offset1 , offset2);
					if( tmp != 0) return tmp;
				}
			}
		}
	...
	void itemsSetLength(int _length){
		if( _length > items_max_length){ 
			...
			items = new ::ss::sna::Node[items_max_length ];
			if( _previous ){
