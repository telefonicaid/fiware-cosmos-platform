...
static void logHookFunction(void* vP, char* text, char type, const char* date, const char* file, int lineNo, const char* fName, int tLev, const char* stre)
{
	...
	header.code    = samson::Message::LogLine;
	header.type    = samson::Message::Msg;
...
Endpoint* workerNew(int ix)
{
	...
	ep = new Endpoint(samson::Endpoint::Worker, alias);
	if (ep == NULL)
		LM_XP(1, ("new Endpoint"));
...
void Network::helloReceived(Endpoint* ep, Message::HelloData* hello, Message::Header* headerP)
{
	...
	if (hostP == NULL)
	{
		...
		epp = endpointLookup((samson::Endpoint::Type) hello->type, hello->ip);
		if (epp)
			LM_W(("Endpoint of type '%s' in host '%s' already exists in endpointList (%s@%s)",
				  endpoint[ME]->typeName((samson::Endpoint::Type) hello->type), hello->ip, epp->name.c_str(), epp->ip));
	}
	else
	{
		epp = endpointLookup((samson::Endpoint::Type) hello->type, hostP);
		if (epp == NULL)
		{
			epp = endpointLookup((samson::Endpoint::Type) hello->type, hello->ip);
			if (epp)
	...
	LM_T(LmtHello, ("--------------------- Got Hello from %s (%s@%s - %s) ------------------------", endpoint[ME]->typeName((samson::Endpoint::Type) hello->type), hello->name, hello->ip, hello->alias));
	...
	LM_T(LmtEndpointSlots, ("Old endpoint slot: %d for '%s' endpoint. New slot: %d", oldSlot, endpoint[ME]->typeName((samson::Endpoint::Type) hello->type), newSlot));

	// LmtHello
	LM_T(LmtEndpointSlots, ("This endpoint is of type '%s', use endpoint slot %d", endpoint[ME]->typeName((samson::Endpoint::Type) hello->type), newSlot));
	...
	LM_T(LmtHello, ("Hello received: %s %s", ep->typeName() , endpoint[ME]->typeName() ));;
...
void Network::logFileSend(Endpoint* ep, Message::MessageCode msgCode, bool oldLogFile)
{
	...
	s = iomMsgSend(ep, endpoint[ME], msgCode, samson::Message::Ack, buf, tot);
	if (s != 0)
		LM_E(("iomMsgError"));
}
...
void Network::msgTreat(void* vP)
{
	...
	case Message::LogLine:
		if (endpoint[ME]->type != samson::Endpoint::Supervisor)
