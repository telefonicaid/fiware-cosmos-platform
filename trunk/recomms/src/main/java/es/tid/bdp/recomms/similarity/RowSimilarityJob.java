package es.tid.bdp.recomms.similarity;
...
import es.tid.bdp.recomms.similarity.measures.VectorSimilarityMeasures;
...
public class RowSimilarityJob extends AbstractJob {
    ...
    public int run(String[] args) throws Exception {
        ...
        int numberOfColumns = Integer.parseInt(parsedArgs
                .get("--numberOfColumns"));
        ...
        int maxSimilaritiesPerRow = Integer.parseInt(parsedArgs
                .get("--maxSimilaritiesPerRow"));
        boolean excludeSelfSimilarity = Boolean.parseBoolean(parsedArgs
                .get("--excludeSelfSimilarity"));
        double threshold = parsedArgs.containsKey("--threshold") ? Double
                .parseDouble(parsedArgs.get("--threshold")) : NO_THRESHOLD;
    ...
    public static class UnsymmetrifyMapper extends
        ...
        protected void map(IntWritable row,
            ...
            TopK<Vector.Element> topKQueue = new TopK<Vector.Element>(
                    maxSimilaritiesPerRow, BY_VALUE);

            Iterator<Vector.Element> nonZeroElements = similarities
                    .iterateNonZero();
            while (nonZeroElements.hasNext()) {
                Vector.Element nonZeroElement = nonZeroElements.next();
                topKQueue.offer(new TIDUtils.TemporaryElement(nonZeroElement));
