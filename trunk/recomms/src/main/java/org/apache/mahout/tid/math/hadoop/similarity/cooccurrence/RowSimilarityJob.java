...
package org.apache.mahout.tid.math.hadoop.similarity.cooccurrence;
...
import org.apache.mahout.tid.math.hadoop.similarity.cooccurrence.Vectors;
import org.apache.mahout.tid.math.hadoop.similarity.cooccurrence.measures.VectorSimilarityMeasure;
import org.apache.mahout.tid.math.hadoop.similarity.cooccurrence.measures.VectorSimilarityMeasures;
...
public class RowSimilarityJob extends AbstractJob {
    ...
    public int run(String[] args) throws Exception {
        ...
        addOption("excludeSelfSimilarity", "ess",
                "compute similarity of rows to themselves?",
                String.valueOf(false));
        addOption("threshold", "tr",
                "discard row pairs with a similarity value below this", false);
        ...
        int numberOfColumns = Integer.parseInt(parsedArgs.get(
                "--numberOfColumns").get(0));
        String similarityClassnameArg = parsedArgs.get("--similarityClassname")
                .get(0);
        String similarityClassname;
        try {
            similarityClassname = VectorSimilarityMeasures.valueOf(
                    similarityClassnameArg).getClassname();
        } catch (IllegalArgumentException iae) {
            similarityClassname = similarityClassnameArg;
        }

        int maxSimilaritiesPerRow = Integer.parseInt(parsedArgs.get(
                "--maxSimilaritiesPerRow").get(0));
        boolean excludeSelfSimilarity = Boolean.parseBoolean(parsedArgs.get(
                "--excludeSelfSimilarity").get(0));
        double threshold = parsedArgs.containsKey("--threshold") ? Double
                .parseDouble(parsedArgs.get("--threshold").get(0))
                : NO_THRESHOLD;
        ...
        if (shouldRunNextPhase(parsedArgs, currentPhase)) {
            ...
            Configuration normsAndTransposeConf = normsAndTranspose
                    .getConfiguration();
        ...
        if (shouldRunNextPhase(parsedArgs, currentPhase)) {
            Job pairwiseSimilarity = prepareJob(weightsPath,
                    pairwiseSimilarityPath, CooccurrencesMapper.class,
                    IntWritable.class, VectorWritable.class,
                    SimilarityReducer.class, IntWritable.class,
            ...
            pairwiseConf.setBoolean(EXCLUDE_SELF_SIMILARITY,
                    excludeSelfSimilarity);
            pairwiseSimilarity.waitForCompletion(true);
        }

        if (shouldRunNextPhase(parsedArgs, currentPhase)) {
            Job asMatrix = prepareJob(pairwiseSimilarityPath, getOutputPath(),
                    UnsymmetrifyMapper.class, IntWritable.class,
                    VectorWritable.class, MergeToTopKSimilaritiesReducer.class,
                    IntWritable.class, VectorWritable.class);
    ...
    public static class VectorNormMapper extends
        ...
        protected void map(IntWritable row, VectorWritable vectorWritable,
            ...
            Iterator<Vector.Element> nonZeroElements = rowVector
                    .iterateNonZero();
            while (nonZeroElements.hasNext()) {
                Vector.Element element = nonZeroElements.next();
                RandomAccessSparseVector partialColumnVector = new RandomAccessSparseVector(
                        Integer.MAX_VALUE);
                partialColumnVector.setQuick(row.get(), element.get());
                ctx.write(new IntWritable(element.index()), new VectorWritable(
                        partialColumnVector));
        ...
        protected void cleanup(Context ctx) throws IOException,
            ...
            ctx.write(new IntWritable(NUM_NON_ZERO_ENTRIES_VECTOR_MARKER),
                    new VectorWritable(nonZeroEntries));
            ctx.write(new IntWritable(MAXVALUE_VECTOR_MARKER),
                    new VectorWritable(maxValues));
        }
    }

    public static class MergeVectorsCombiner extends
            Reducer<IntWritable, VectorWritable, IntWritable, VectorWritable> {
        @Override
        protected void reduce(IntWritable row,
                Iterable<VectorWritable> partialVectors, Context ctx)
    ...
    public static class MergeVectorsReducer extends
        ...
        protected void setup(Context ctx) throws IOException,
            ...
            numNonZeroEntriesPath = new Path(ctx.getConfiguration().get(
                    NUM_NON_ZERO_ENTRIES_PATH));
        ...
        protected void reduce(IntWritable row,
            ...
            if (row.get() == NORM_VECTOR_MARKER) {
                ...
                Vectors.write(partialVector, maxValuesPath,
    ...
    public static class SimilarityReducer extends
        ...
        protected void reduce(IntWritable row,
            ...
            while (partialDotsIterator.hasNext()) {
                ...
                Iterator<Vector.Element> nonZeroElements = toAdd
                        .iterateNonZero();
                while (nonZeroElements.hasNext()) {
            ...
            while (dotsWith.hasNext()) {
                ...
                if (Math.abs(similarityValue) >= treshold) {
    ...
    public static class UnsymmetrifyMapper extends
        ...
        protected void setup(Mapper.Context ctx) throws IOException,
                InterruptedException {
            maxSimilaritiesPerRow = ctx.getConfiguration().getInt(
                    MAX_SIMILARITIES_PER_ROW, 0);
            Preconditions.checkArgument(maxSimilaritiesPerRow > 0,
                    "Incorrect maximum number of similarities per row!");
        }
        ...
        protected void map(IntWritable row,
            ...
            TopK<Vector.Element> topKQueue = new TopK<Vector.Element>(
                    maxSimilaritiesPerRow, Vectors.BY_VALUE);

            Iterator<Vector.Element> nonZeroElements = similarities
                    .iterateNonZero();
            while (nonZeroElements.hasNext()) {
                Vector.Element nonZeroElement = nonZeroElements.next();
                ...
                ctx.write(new IntWritable(nonZeroElement.index()),
                        new VectorWritable(transposedPartial));
            }
            Vector topKSimilarities = similarities.like();
            for (Vector.Element topKSimilarity : topKQueue.retrieve()) {
                topKSimilarities.setQuick(topKSimilarity.index(),
                        topKSimilarity.get());
            }
    ...
    public static class MergeToTopKSimilaritiesReducer extends
        ...
        @Override
        protected void reduce(IntWritable row,
                Iterable<VectorWritable> partials, Context ctx)
                throws IOException, InterruptedException {
            Vector allSimilarities = Vectors.merge(partials);
            Vector topKSimilarities = Vectors.topKElements(
                    maxSimilaritiesPerRow, allSimilarities);
            ctx.write(row, new VectorWritable(topKSimilarities));
        }
    }
