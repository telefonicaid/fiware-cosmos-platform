...
namespace samson {
  ...
  ProcessItemIsolated::ProcessItemIsolated(SamsonWorker* samson_worker, size_t worker_task_id , const std::string& operation , const std::string& concept)
  ...
  void ProcessItemIsolated::runExchangeMessages() {
    ...
    while (true) {
      ...
      if (c != au::OK) {
        ...
        error_.set(au::str("Third party operation '%s' has crashed [%s] - [ Error code %s ]"
                           ,operation_.c_str()
                           ,concept().c_str()
                           ,au::status(c))
                   );
