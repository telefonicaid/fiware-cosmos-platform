...
namespace sna{
	...
	static inline int size(char *data){
		...
		{ //Getting size of vector links
			...
			for (int i = 0 ; i < (int)_length ; i++){
				offset += ::ss::sna::Link::size(data+offset);
			}
		}
		...
		{ // Comparing vector links
			...
			*offset2 += ss::staticVarIntParse( data2+(*offset2) , &_length2 );
			...
			for (int i = 0 ; i < (int)_length1 ; i++){
				{ // comparing links[i]
					int tmp = ::ss::sna::Link::compare(data1,data2,offset1 , offset2);
					if( tmp != 0) return tmp;
				}
			}
		}
	...
	void linksSetLength(int _length){
		if( _length > links_max_length){ 
			...
			links = new ::ss::sna::Link[links_max_length ];
			if( _previous ){
