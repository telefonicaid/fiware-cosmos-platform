...
namespace sna_light{
	...
	::ss::sna_light::Clique_Link *links;
	int links_length;
	...
	static inline int size(char *data){
		...
		{ //Getting size of vector links
			size_t _length;
			offset += ss::staticVarIntParse( data+offset , &_length );
			::ss::sna_light::Clique_Link _tmp;
			for (int i = 0 ; i < (int)_length ; i++){
				offset += ::ss::sna_light::Clique_Link::size(data+offset);
			}
		}
		return offset;
	}
		...
		{ // Comparing vector links
			...
			*offset2 += ss::staticVarIntParse( data2+(*offset2) , &_length2 );
			...
			for (int i = 0 ; i < (int)_length1 ; i++){
				{ // comparing links[i]
					int tmp = ::ss::sna_light::Clique_Link::compare(data1,data2,offset1 , offset2);
					if( tmp != 0) return tmp;
				}
			}
		}
	...
	void linksSetLength(int _length){
		if( _length > links_max_length){ 
			::ss::sna_light::Clique_Link *_previous = links;
			...
			links = new ::ss::sna_light::Clique_Link[links_max_length ];
			if( _previous ){
	...
	::ss::sna_light::Clique_Link* linksAdd(){
