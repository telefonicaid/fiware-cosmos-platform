...
#define APPLICATION_VERSION	"1.0-14"
...
static char* StrUpr ( char* pszInput )
{
	unsigned int uiLength = strlen ( pszInput ) ;
	for ( unsigned int uiLoop = 0 ; uiLoop < uiLength ; uiLoop ++ )
		pszInput [ uiLoop ] = toupper ( pszInput [ uiLoop ] ) ;
	return pszInput ;
}
...
private:
	//CLoggerStaticInfo(string strLogFileNameWithoutPath="droned", string strLogPathWithoutFileName="/home/droned/");
	CLoggerStaticInfo(string strLogFileNameWithoutPath="csCFD", string strLogPathWithoutFileName="/var/log/csCFD");
...
#define CLoggerDefineFunction(FunctionName)																																			\
	template<class T1> void Log##FunctionName(const T1 &varT1)																																\
	{																																												\
		ostringstream ossMessage;																																					\
		ossMessage << varT1;																																						\
		Log(LL_##FunctionName, #FunctionName, ossMessage.str().c_str());																											\
	}																																												\
	template<class T1, class T2> void Log##FunctionName(const T1 &varT1, const T2 &varT2)																											\
	{																																												\
		ostringstream ossMessage;																																					\
		ossMessage << varT1 << HD_LogParameterSeparator << varT2;																													\
		Log(LL_##FunctionName, #FunctionName, ossMessage.str().c_str());																											\
	}																																												\
	template<class T1, class T2, class T3> void Log##FunctionName(const T1 &varT1, const T2 &varT2, const T3 &varT3)																						\
	{																																												\
		ostringstream ossMessage;																																					\
		ossMessage << varT1 << HD_LogParameterSeparator << varT2 << HD_LogParameterSeparator << varT3;																				\
		Log(LL_##FunctionName, #FunctionName, ossMessage.str().c_str());																											\
	}																																												\
	template<class T1, class T2, class T3, class T4> void Log##FunctionName(const T1 &varT1, const T2 &varT2, const T3 &varT3, const T4 &varT4)																	\
	{																																												\
		ostringstream ossMessage;																																					\
		ossMessage << varT1 << HD_LogParameterSeparator << varT2 << HD_LogParameterSeparator << varT3 << HD_LogParameterSeparator << varT4;											\
		Log(LL_##FunctionName, #FunctionName, ossMessage.str().c_str());																											\
	}																																												\
	template<class T1, class T2, class T3, class T4, class T5> void Log##FunctionName(const T1 &varT1, const T2 &varT2, const T3 &varT3, const T4 &varT4, const T5 &varT5)												\
	{																																												\
		ostringstream ossMessage;																																					\
