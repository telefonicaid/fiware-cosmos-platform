...
/*
    ...
    {
      error_.set( "ProcessItem canceled");
      ...
      response->set_code( samson::gpb::MessagePlatformProcess_Code_code_kill );
...
*/
    ...
    switch (  message->code()  )
    {
      ...
      case samson::gpb::MessageProcessPlatform_Code_code_user_error:
      {
        ...
        if (au::writeGPB(pipeFdPair2[1], response) != au::OK)
        {
          LM_E(("Error sending user error, code(%d),  (pipeFdPair2[1]:%d), error message:%s ",  response->code(), pipeFdPair2[1], error_.GetMessage().c_str()));
        }
        delete response;
    ...
    // Continuous read of messages and perform required actions
      ...
      if( c != au::OK )
      {
        // Not possible to read the message for any reason
				...
				error_.set( au::str( "Third party operation '%s' has crashed - [ Error code %s ]", processItemIsolated_description.c_str() ,  au::status(c) ) );
        return;
      }
  ...
  void ProcessItemIsolated::sendMessageProcessPlatform(samson::gpb::MessageProcessPlatform *message )
  {
	...
	{
    // Change mode how logs are generated ( we are in aseparated process )
