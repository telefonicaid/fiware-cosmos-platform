...
namespace sna_light{
	...
	class Vector_Node_base : public samson::DataInstance{

	public:
	::samson::sna_light::Node *items;
	int items_length;
	int items_max_length;

	Vector_Node_base() : samson::DataInstance(){
		items_length=0;
		items_max_length=0;
		items = NULL;
	}
	...
	static inline int size(char *data){
		...
		{ //Getting size of vector items
			size_t _length;
			offset += samson::staticVarIntParse( data+offset , &_length );
			::samson::sna_light::Node _tmp;
			for (int i = 0 ; i < (int)_length ; i++)
			{ //Sizing items
				offset += ::samson::sna_light::Node::size(data+offset);
			}
	...
	inline static int compare(char * data1 , char *data2 , size_t *offset1 , size_t *offset2 ){
		{ // Comparing vector items
			...
			*offset2 += samson::staticVarIntParse( data2+(*offset2) , &_length2 );
			...
			{ // comparing items[i]
				int tmp = ::samson::sna_light::Node::compare(data1,data2,offset1 , offset2);
				if( tmp != 0) return tmp;
			}   //  items[i] compared 
	...
	void itemsSetLength(int _length){
		if( _length > items_max_length){ 
			::samson::sna_light::Node *_previous = items;
			...
			items = new ::samson::sna_light::Node[items_max_length ];
			if( _previous ){
	...
	::samson::sna_light::Node* itemsAdd(){
