...
public class Vectors {
    ...
    public static Vector maybeSample(Vector original, int sampleSize) {
        ...
        Iterator<Vector.Element> sampledElements = new FixedSizeSamplingIterator<Vector.Element>(
                sampleSize, original.iterateNonZero());
    ...
    public static Vector merge(Iterable<VectorWritable> partialVectors) {
        ...
        while (vectors.hasNext()) {
            ...
            if (v != null) {
                Iterator<Vector.Element> nonZeroElements = v.get()
                        .iterateNonZero();
                while (nonZeroElements.hasNext()) {
                    Vector.Element nonZeroElement = nonZeroElements.next();
                    accumulator.setQuick(nonZeroElement.index(),
                            nonZeroElement.get());
                }
            }
        }
    ...
    public static Vector.Element[] toArray(VectorWritable vectorWritable) {
        Vector.Element[] elements = new Vector.Element[vectorWritable.get()
                .getNumNondefaultElements()];
        int k = 0;
        Iterator<Vector.Element> nonZeroElements = vectorWritable.get()
                .iterateNonZero();
        while (nonZeroElements.hasNext()) {
            Vector.Element nonZeroElement = nonZeroElements.next();
            elements[k++] = new TemporaryElement(nonZeroElement.index(),
                    nonZeroElement.get());
        }
        return elements;
    }

    public static void write(Vector vector, Path path, Configuration conf)
            throws IOException {
        write(vector, path, conf, false);
    }

    public static void write(Vector vector, Path path, Configuration conf,
            boolean laxPrecision) throws IOException {
    ...
    public static OpenIntIntHashMap readAsIntMap(Path path, Configuration conf)
            throws IOException {
        FileSystem fs = FileSystem.get(path.toUri(), conf);
    ...
    public static OpenIntIntHashMap readAsIntMap(DataInput in)
            throws IOException {
        ...
        Preconditions.checkState(!dense && !sequential,
                "Only for reading sparse vectors!");
