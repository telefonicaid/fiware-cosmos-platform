...
namespace samson {
  namespace stream {
    ...
    : ProcessIsolated( samson_worker
                      ...
                      , au::str("WorkerTask %lu: %s", id, stream_operation.operation().c_str())
                      , get_type(operation) )
    {
    ...
    void WorkerTask::generateTXT(TXTWriter *writer) {
      ...
      OperationTraces operation_traces(au::str("[%lu] Parserout %s", worker_task_id(), process_item_description().c_str()), 100);
...
#pragma mark
    ...
    void WorkerTask::generateKeyValues_reduce(samson::ProcessWriter *writer)
    {
      ...
      OperationTraces operation_traces(au::str("[%lu] reduce %s", worker_task_id(),  process_item_description().c_str()), 100);
    ...
    std::string WorkerTask::str() {
      return au::str("Task %lu : %s %s %s"
                     , worker_task_id()
                     , stream_operation_->name().c_str()
                     , stream_operation_->operation().c_str()
                     , range_.str().c_str() );
    }
  }
}
