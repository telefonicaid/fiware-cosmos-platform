...
int main(int argC, const char *argV[])
{
	...
	listenFd = iomServerOpen(port);

	LM_M(("Creating accepter on fd %d (port %d)", listenFd, port));
	Accepter accepter(listenFd, port);
	...
	if ((int) spawnerList[0] != 0)
	{
		...
		for (ix = 1; ix <= (int) spawnerList[0]; ix++)
		{
			...
			LM_M(("spawner %d: '%s'", ix, spawnerList[ix]));
			fd = iomConnect(spawnerList[ix], SPAWNER_PORT);
			if (fd != -1)
				logProviderAdd("spawner", spawnerList[ix], fd);
		}
	}
	...
	if (strcmp(controller, (char*) NOC) != 0)
	{
		...
		if (fd != -1)
			logProviderAdd("controller", controller, fd);
	}
	...
	if ((int) workerList[0] != 0)
	{
		...
		for (ix = 1; ix <= (int) workerList[0]; ix++)
		{
			...
			LM_M(("worker %d: '%s'", ix, workerList[ix]));
			fd = iomConnect(workerList[ix], WORKER_PORT);
			if (fd != -1)
				logProviderAdd("worker", workerList[ix], fd);
		}
	}
