...
class UnfilteredExceptionRenderer[T] extends ExceptionRenderer[ResponseFunction[T]] {
  override protected def withAuthHeader(response: ResponseFunction[T]): ResponseFunction[T] =
    response ~> WWWAuthenticate("""Basic realm="Infinity", Bearer realm="Infinity"""")
