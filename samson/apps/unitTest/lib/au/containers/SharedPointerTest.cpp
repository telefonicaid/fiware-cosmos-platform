...
TEST(au_containers_SharedPointer , assignation )
{
   ...
   EXPECT_EQ( a.reference_count() , 2 ) <<  "Error in au::SharedPointer assignation";
   EXPECT_EQ( b.reference_count() , 2 ) <<  "Error in au::SharedPointer assignation";
...
TEST(au_containers_SharedPointer , operators )
{
   ...
   EXPECT_EQ( a.shared_object() == c.shared_object() , false  ) << "Error in au::SharedPointer::shared_object() ";   
   EXPECT_EQ( a==c , false ) << "Error in au::SharedPointer operator == ";
   EXPECT_EQ( a!=c , true ) << "Error in au::SharedPointer operator != ";

   EXPECT_EQ( a == b.shared_object() , true ) << "Error in au::SharedPointer operator ==";
   EXPECT_EQ( a == c.shared_object() , false ) << "Error in au::SharedPointer operator ==";
...
TEST(au_containers_SharedPointer , dynamic_castings )
{
   ...
   au::SharedPointer< TestDerived2 > c = base_a.dynamic_pointer_cast<TestDerived2>( );


   EXPECT_EQ( a == b , true ) << "Error in au::SharedPointer dynamic casting (1)";
   EXPECT_EQ( c == NULL , true ) << "Error in au::SharedPointer dynamic casting (2)";
