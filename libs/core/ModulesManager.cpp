...
namespace ss
{
	ModulesManager *ssContainerManager  = NULL;

	ModulesManager*  ModulesManager::shared()
	{
		if( !ssContainerManager)
			ssContainerManager = new ModulesManager();
		
		return ssContainerManager;
	}
	...
	void ModulesManager::addModules()
	{
		...
		{
			Data *data = new Data( "String" , ss::system::String::createFunction );
			data->setCompareByValueFunction( system::String::compareByValue );
			tmp->add( data );
		}
		
		{
			Data *data = new Data( "Void" , ss::system::Void::createFunction );
			data->setCompareByValueFunction( NULL );	// No comparison by value
			tmp->add( data );
		}
	...
	void ModulesManager::addModule( std::string path )
	{
		...
		if( mkr && getVersionPointer )
		{
			...
			if ( !module )
			{
				SSLogger::log( SSLogger::warning, "Not possible to load module at path" + path + " (not container found)" );
				dlclose(hndl);
				return;
			}
