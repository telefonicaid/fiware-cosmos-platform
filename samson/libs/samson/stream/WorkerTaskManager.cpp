...
void WorkerTaskManager::notify(engine::Notification *notification) {
  if (notification->isName(notification_process_request_response)) {
    ...
    if (task_base == NULL) {
      AU_W( logs.task_manager, ("Notification of an unknown finished system queue tasks (task = %lu ).Ignoring", id));
      return;
    }
    
    AU_M( logs.task_manager, ("Finished task %lu (%s)", id , task_base->str().c_str() ));
...
bool WorkerTaskManager::runNextWorkerTasksIfNecessary() {
  ...
  for (size_t i = 0; i < tasks.size(); ++i) {
    ...
    if (base_task->is_ready()) {
      ...
      AU_M(logs.task_manager, ("Scheduled task %lu (%s)", task_id , base_task->str().c_str() ));
...
void WorkerTaskManager::review_stream_operations() {
  ...
  if (memory_usage >= 1.0) {
    AU_SW(("Not scheduling new stream-tasks since memory usage is %s >= 100%"
