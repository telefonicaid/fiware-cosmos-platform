...
void LogCentral::write(Log *log) {
  ...
  if (direct_mode) {
    if (socket_connection)
      log->write(socket_connection); else if (local_file_descriptor)
      log->write(local_file_descriptor); return;
  }
...
void LogCentral::write_to_server_or_file(Log *log) {
  ...
  if (!socket_connection && ( host != "" )  && ( time >= time_reconnect )) {
    ...
    if (s != au::OK) {
      ...
      if (time == 0)
        time_reconnect = 120; if (time_reconnect < time)
        time_reconnect = time; time_reconnect *= 2;           // Increase time to reconnect
