...
#ifndef _H_SAMSON_sna_parse_cdrs
...
public:
	...
	bool getCDRFromLine_TME( char *line, samson::system::UInt*node, CDR* cdr )
	{
		...
		cdr->time.getTimeUTCFromCalendar(&timeExpanded);
	...
	void parseLines( char *line, samson::KVWriter *writer )
	{
		...
		if( (this->*_getCDRFunction)(line , &number ,  &cdr ) )
		{
			...
			// Now it is done locally, as a by-product of cdr->time.getTimeUTCFromCalendar(&timeExpanded);
			//cdr.time.getCalendarFromTimeUTC(&timeExpanded);
			//cdr.week_day = timeExpanded.tm_wday;
