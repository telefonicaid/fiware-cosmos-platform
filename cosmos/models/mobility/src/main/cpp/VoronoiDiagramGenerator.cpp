...
void VoronoiDiagramGenerator::pushGraphEdge(int indv1, double xv1, double yv1, int indv2, double xv2, double yv2, int inds1, int inds2)
{
...
void VoronoiDiagramGenerator::line(int indv1, double xv1, double yv1, int indv2, double xv2, double yv2, int inds1, int inds2)
{	
	pushGraphEdge(indv1, xv1, yv1, indv2, xv2, yv2, inds1, inds2);
...
void VoronoiDiagramGenerator::clip_line(struct Edge *e)
{
	...
	if (cambio) {
        line((e->ep[0] != NULL ? e->ep[0]->sitenbr : -1), x2, y2, (e->ep[1] != NULL ? e->ep[1]->sitenbr : -1), x1, y1,
             e->reg[0]->sitenbr, e->reg[1]->sitenbr);
    }
    else {
        line((e->ep[0] != NULL ? e->ep[0]->sitenbr : -1), x1, y1, (e->ep[1] != NULL ? e->ep[1]->sitenbr : -1), x2, y2,
             e->reg[0]->sitenbr, e->reg[1]->sitenbr);
    }
}
...
void VoronoiDiagramGenerator::postproceso(struct pto_origen **listaPuntos_sal, struct vertice **listaVertices_sal,
                 VoronoiDiagramGenerator *vdg, int count, int count_ext,
                 double xmin, double xmax, double ymin, double ymax, double distmin)
{       
    int count_total = count + count_ext;
    int indv1, indv2, inds1, inds2;
    double xv1, yv1, xv2, yv2;
    double longi_lado;

    struct pto_origen *listaPuntos;
    struct vertice *listaVertices;
    
    int num_vertices = 0;
    size_t ind_vertices = 0;
    
    *listaPuntos_sal = listaPuntos = (struct pto_origen *) malloc(sizeof(struct pto_origen) * count_total);
    
    for (int i =0; (i < count_total); i++)
    {
        listaPuntos[i].num_lados = 0;
        listaPuntos[i].num_adyacentes = 0;
        listaPuntos[i].indice_lados = 0;
        listaPuntos[i].indice_adyacentes = 0;
        listaPuntos[i].poligono = NULL;
        listaPuntos[i].ptos_adyacentes = NULL;
        listaPuntos[i].area = 0.0;
    }
    ...
    printf("\n-------------------------------\n");
    /* */
	while(vdg->getNext(indv1, xv1, yv1, indv2, xv2, yv2, inds1, inds2))
	{
		/* ### *
        printf("Line (%d:%f,%f)->(%d:%f,%f) between sites %d %d\n",
               indv1, xv1, yv1, indv2, xv2, yv2, inds1, inds2);
        /* */
        
        longi_lado = sqrt((xv1 - xv2) * (xv1 - xv2) + (yv1 - yv2) * (yv1 - yv2));
        
        /* Los lados que provengan del infinito o vayan al infinito y tengan longitud nula no son
         tenidos en cuenta. Todos los demás sí. */
        if ((longi_lado > distmin) ||
            ((indv1 != -1) && (indv2 != -1))) {
            listaPuntos[inds1].num_lados++;
            listaPuntos[inds2].num_lados++;
        
            /* Sólo consideramos adyacencias con otros polígonos cuando encontramos un lado que tiene longitud no nula. Los lados con longitud no nula se usan para contar por exceso el número de vértices (dos por lado, aunque muchos coincidirán).
             Tampoco consideramos adyacencias con los polígonos añadidos para limitar el contorno. */
            if (longi_lado > distmin) {
                if (inds2 < count) {
                    listaPuntos[inds1].num_adyacentes++;
                }
                if (inds1 < count) {
                    listaPuntos[inds2].num_adyacentes++;
                }
                num_vertices += 2;
            }
        }
    ...
    /* Reservamos dos lados más por cada polígono, por si hay que añadir un vértice más (caso de incluir
     un vértice del rectángulo que limita el diagrama. */
    for (int i =0; (i < count_total); i++)
    {
        listaPuntos[i].poligono = (struct lado *) malloc((listaPuntos[i].num_lados + 2) * sizeof(struct lado));
        listaPuntos[i].ptos_adyacentes = (int *) malloc(listaPuntos[i].num_adyacentes * sizeof(int));
    }
    
    /* num_vertices es el número de vértices.
     Reservamos 4 más, por las 4 esquinas del rectángulo que limita el diagrama (por si acaso).
     Habrá vértices en exceso, porque cada vértice pertenece al menos a dos lados. Luego ajustamos la cuenta. */
    *listaVertices_sal = listaVertices = (struct vertice *) malloc((num_vertices + 4) * sizeof(struct vertice));
    
    vdg->resetIterator();
    
    /* Voy a guardar los vértices, ajustando más las cuentas de los que son.
     También se aprovecha para guardar la información de los lados y de los puntos (polígonos) adyacentes. */
    while(vdg->getNext(indv1, xv1, yv1, indv2, xv2, yv2, inds1, inds2))        
	{
        struct vertice un_vertice;
        size_t ind_vertices_ant;
        struct vertice *nuevo_vertice1;
        struct vertice *nuevo_vertice2;
        int ind_lado;
        int ind_adyacente;
        ...
        if ((longi_lado > distmin) ||
            ((indv1 != -1) && (indv2 != -1))) {
            un_vertice.coordx = xv1;
            un_vertice.coordy = yv1;
            un_vertice.ind_orig = indv1;
            un_vertice.ind_final = indv1;
            
            ind_vertices_ant = ind_vertices;
            nuevo_vertice1 = (struct vertice *) lsearch(&un_vertice, listaVertices, &ind_vertices, sizeof (struct vertice), compara_vertices_orig);
            if (ind_vertices > ind_vertices_ant) {
                nuevo_vertice1->ind_final = (int) ind_vertices_ant;
            }

            un_vertice.coordx = xv2;
            un_vertice.coordy = yv2;
            un_vertice.ind_orig = indv2;
            un_vertice.ind_final = indv2;
            
            ind_vertices_ant = ind_vertices;
            nuevo_vertice2 = (struct vertice *) lsearch(&un_vertice, listaVertices, &ind_vertices, sizeof (struct vertice), compara_vertices_orig);
            if (ind_vertices > ind_vertices_ant) {
                nuevo_vertice2->ind_final = (int) ind_vertices_ant;
            }
        
            //Voy guardando los lados y los puntos adyacentes.
            ind_lado = listaPuntos[inds1].indice_lados;
            ind_adyacente = listaPuntos[inds1].indice_adyacentes;
        
            listaPuntos[inds1].poligono[ind_lado].indv1_orig = indv1;
            listaPuntos[inds1].poligono[ind_lado].indv2_orig = indv2;
            listaPuntos[inds1].poligono[ind_lado].indv1_final = nuevo_vertice1->ind_final;
            listaPuntos[inds1].poligono[ind_lado].indv2_final = nuevo_vertice2->ind_final;
            listaPuntos[inds1].indice_lados++;
            
            if ((longi_lado > distmin) && (inds2 < count)) {
                listaPuntos[inds1].ptos_adyacentes[ind_adyacente] = inds2;
                listaPuntos[inds1].indice_adyacentes++;
            }
        
            //Y tratamos el mismo lado dado la vuelta.        
            ind_lado = listaPuntos[inds2].indice_lados;
            ind_adyacente = listaPuntos[inds2].indice_adyacentes;

            listaPuntos[inds2].poligono[ind_lado].indv1_orig = indv2;
            listaPuntos[inds2].poligono[ind_lado].indv2_orig = indv1;
            listaPuntos[inds2].poligono[ind_lado].indv1_final = nuevo_vertice2->ind_final;
            listaPuntos[inds2].poligono[ind_lado].indv2_final = nuevo_vertice1->ind_final;
            listaPuntos[inds2].indice_lados++;
            
            if ((longi_lado > distmin) && (inds1 < count)) {
                listaPuntos[inds2].ptos_adyacentes[ind_adyacente] = inds1;
                listaPuntos[inds2].indice_adyacentes++;
            }
        }
    }
    num_vertices = (int) ind_vertices;
    
    /* ### *
    for (int i = 0; (i < count_total); i++)
    {
        printf("Punto: %d Num lados: %d :", i, listaPuntos[i].num_lados);
        for(int j = 0; j < listaPuntos[i].num_lados; j++)
        {
            printf(" (%d,%d),",listaPuntos[i].poligono[j].indv1_orig,listaPuntos[i].poligono[j].indv2_orig);
        }
        printf("\n");
        printf("Punto: %d Num lados: %d :", i, listaPuntos[i].num_lados);
        for(int j = 0; j < listaPuntos[i].num_lados; j++)
        {
            printf(" (%d,%d),",listaPuntos[i].poligono[j].indv1_final,listaPuntos[i].poligono[j].indv2_final);
        }
        printf("\n");
        printf("Punto: %d Num lados: %d :", i, listaPuntos[i].num_lados);
        for(int j = 0; j < listaPuntos[i].num_lados; j++)
        {
            if (listaPuntos[i].poligono[j].indv1_final != -1) {
                printf(" (%f,%f),",
                       listaVertices[listaPuntos[i].poligono[j].indv1_final].coordx,
                       listaVertices[listaPuntos[i].poligono[j].indv1_final].coordy);
            }
            else {
                printf(" (Inf,Inf),");
            }
        ...
        printf("\n");
        printf("Punto: %d Área: %f\n", i, listaPuntos[i].area);
        printf("Punto: %d Num ady: %d :", i, listaPuntos[i].num_adyacentes);
        for (int j = 0; (j < listaPuntos[i].num_adyacentes); j++) {
            printf(" %d,", listaPuntos[i].ptos_adyacentes[j]);
        }
        printf("\n");
    }
    ...
    {
        printf("Vertice %d %d: (%f,%f)\n",
               listaVertices[i].ind_orig, listaVertices[i].ind_final, listaVertices[i].coordx,listaVertices[i].coordy);
    }
    /* */
    
    /* Repasamos los polígonos para quitar y meter lados.
     En ocasiones, habrá que recurrir a las esquinas del rectángulo que encierra el diagrama. */
    for (int i = 0; (i < count_total); i++) {
        int j, k, l;
        int encontrado_sig;
        int sig;
        struct lado lado_tmp;
        struct lado *p_lado1;
        struct lado *p_lado2;
        int encierra_esquina;
        struct vertice *p_vertice1;
        struct vertice *p_vertice2;
        size_t ind_vertices_ant;
        struct vertice un_vertice;
        struct vertice *nuevo_vertice;
        struct lado *p_lado_aux;
        
        /* Ordenamos los lados del polígono. */
        for(j = 0; (j < listaPuntos[i].num_lados); j++) {
            encontrado_sig = 0;
            for (k = j + 1; (k <= listaPuntos[i].num_lados); k++) {
                l = k % listaPuntos[i].num_lados;
                if (listaPuntos[i].poligono[j].indv2_orig ==
                    listaPuntos[i].poligono[l].indv1_orig) {
                        encontrado_sig = 1;
                        break;
                }
            }
            if (encontrado_sig) {
                sig = (j + 1) % listaPuntos[i].num_lados;
                if (l != sig) {
                    if (l < j) {
                        lado_tmp = listaPuntos[i].poligono[j];
                        memmove(listaPuntos[i].poligono + l + 1, listaPuntos[i].poligono + l, (j - l) * sizeof(struct lado));
                        listaPuntos[i].poligono[l] = lado_tmp;
                    }
                    else {
                        lado_tmp = listaPuntos[i].poligono[sig];
                        listaPuntos[i].poligono[sig] = listaPuntos[i].poligono[k];
                        listaPuntos[i].poligono[k] = lado_tmp;
                    }
                }
            }
        }
        
        /* Eliminamos los lados con longitud 0. */
        for (int j = 0; (j < listaPuntos[i].num_lados); j++) {
            p_lado1 = listaPuntos[i].poligono + j;
            
            if (p_lado1->indv1_final == p_lado1->indv2_final) {
                memmove(p_lado1, p_lado1 + 1, (listaPuntos[i].num_lados - j - 1) * sizeof(struct lado));
                listaPuntos[i].num_lados--;
                j--;
            }
        }
        
        /* Arreglamos las discontinuidades en la conexión entre los lados. */
        p_lado1 = listaPuntos[i].poligono;
        for (int j = 1; (j <= listaPuntos[i].num_lados); j++) {
            int k = j % listaPuntos[i].num_lados;
            p_lado2 = listaPuntos[i].poligono + k;
            if (p_lado1->indv2_final != p_lado2->indv1_final) {
                
                /* Se comprueba si la unión de los puntos entre los que se ha detectado la discontinuidad deja a su izquierda algún vértice del rectángulo que encierra el diagrama. Si es así, hay que incluir este vértice en el polígono, y añadir dos lados para unir este vértice con los puntos entre los que se ha detectado la discontinuidad. Si no se deja a la izquierda ningún vértice del rectángulo, sólo hay que añadir un lado para unir los puntos entre los que se ha detectado la discontinuidad. */
                p_vertice1 = listaVertices + p_lado1->indv2_final;
                p_vertice2 = listaVertices + p_lado2->indv1_final;
                    
                encierra_esquina = 0;
                
                un_vertice.ind_orig = -1;
                un_vertice.coordx = xmin;
                un_vertice.coordy = ymin;
                if (isLeft(p_vertice1, p_vertice2, &un_vertice) > 0) {
                    encierra_esquina = 1;
                }
                else {
                    un_vertice.coordx = xmin;
                    un_vertice.coordy = ymax;
                    if (isLeft(p_vertice1, p_vertice2, &un_vertice) > 0) {
                        encierra_esquina = 1;
                    }
                    else {
                        un_vertice.coordx = xmax;
                        un_vertice.coordy = ymax; 
                        if (isLeft(p_vertice1, p_vertice2, &un_vertice) > 0) {
                            encierra_esquina = 1;
                        }
                ...
                if (encierra_esquina) {
                    ind_vertices_ant = ind_vertices;
                    nuevo_vertice = (struct vertice *) lsearch(&un_vertice, listaVertices, &ind_vertices, sizeof(struct vertice), compara_vertices_orig);
                    if (ind_vertices > ind_vertices_ant) {
                        nuevo_vertice->ind_final = (int) ind_vertices_ant;
                    }
                    
                    p_lado_aux = p_lado1 + 1;
                    memmove(p_lado_aux + 2, p_lado_aux, (listaPuntos[i].num_lados - j) * sizeof(struct lado));
                    listaPuntos[i].num_lados += 2;
                    j += 2;
                    k = j % listaPuntos[i].num_lados;
                    p_lado2 = listaPuntos[i].poligono + k;
                    
                    p_lado_aux->indv1_orig = p_lado1->indv2_orig;
                    p_lado_aux->indv2_orig = nuevo_vertice->ind_orig;
                    p_lado_aux->indv1_final = p_lado1->indv2_final;
                    p_lado_aux->indv2_final = nuevo_vertice->ind_final;
                    p_lado_aux++;
                    p_lado_aux->indv1_orig = nuevo_vertice->ind_orig;
                    p_lado_aux->indv2_orig = p_lado2->indv1_orig;
                    p_lado_aux->indv1_final = nuevo_vertice->ind_final;
                    p_lado_aux->indv2_final = p_lado2->indv1_final;
                }
                else {
                    p_lado_aux = p_lado1 + 1;
                    memmove(p_lado_aux + 1, p_lado_aux, (listaPuntos[i].num_lados - j) * sizeof(struct lado));
                    listaPuntos[i].num_lados++;
                    j++;
                    k = j % listaPuntos[i].num_lados;
                    p_lado2 = listaPuntos[i].poligono + k;
        ...
        /* Y ahora vamos a obtener el área de cada polígono. */
        for (int j = 0; (j < listaPuntos[i].num_lados); j++) {
            p_lado1 = listaPuntos[i].poligono + j;
            listaPuntos[i].area += (listaVertices[p_lado1->indv1_final].coordy *
                                    listaVertices[p_lado1->indv2_final].coordx) -
                                   (listaVertices[p_lado1->indv1_final].coordx *
                                    listaVertices[p_lado1->indv2_final].coordy);
        }
        listaPuntos[i].area *= 0.5;
    }
    num_vertices = (int) ind_vertices;
...
int compara_vertices_orig(const void *a, const void *b) {
    struct vertice *punto_a = (struct vertice *) a;
    struct vertice *punto_b = (struct vertice *) b;
    
    if (sqrt((punto_a->coordx - punto_b->coordx) * (punto_a->coordx - punto_b->coordx) +
             (punto_a->coordy - punto_b->coordy) * (punto_a->coordy - punto_b->coordy))
        <= DISTMIN) {
        return(0);
    }
    else {
...
double VoronoiDiagramGenerator::isLeft(struct vertice *P0, struct vertice *P1, struct vertice *P2)
{
    return(P1->coordx - P0->coordx)*(P2->coordy - P0->coordy) - (P2->coordx - P0->coordx)*(P1->coordy - P0->coordy);
}



void VoronoiDiagramGenerator::junta_ent_cont(double **xValues, double **yValues, double *xValues_ext, double *yValues_ext,
                    int count, int count_ext) 
{
    ...
    if ((*xValues = (double *) realloc(*xValues, (count + count_ext) * sizeof(double))) == NULL) {
        perror("junta_ent_cont. No puedo reservar memoria");
        exit(1);
    }
    if ((*yValues = (double *) realloc(*yValues, (count + count_ext) * sizeof(double))) == NULL) {
        perror("junta_ent_cont. No puedo reservar memoria");
        exit(1);
    }
    
    for (i = 0,
         p_aux_x = *xValues + count, p_aux_y = *yValues + count,
         p_aux_x_ext = xValues_ext, p_aux_y_ext = yValues_ext;
         (i < count_ext);
         i++, p_aux_x++, p_aux_y++, p_aux_x_ext++, p_aux_y_ext++) {
        *p_aux_x = *p_aux_x_ext;
        *p_aux_y = *p_aux_y_ext;
    }
    free(xValues_ext);
...
void VoronoiDiagramGenerator::calc_lim(double *xmin, double*xmax, double *ymin, double *ymax, double *xValues, double *yValues, int count) 
{
    int i;
    double xmin_loc, xmax_loc, ymin_loc, ymax_loc, rango_x, rango_y;
    
    xmin_loc = xmax_loc = *xValues;
    ymin_loc = ymax_loc = *yValues;
    for (i = 1; (i < count); i++) {
        if (xValues[i] < xmin_loc) {
            xmin_loc = xValues[i];
        }
        else if (xValues[i] > xmax_loc) {
            xmax_loc = xValues[i];
        }
        if (yValues[i] < ymin_loc) {
            ymin_loc = yValues[i];
        }
        else if (yValues[i] > ymax_loc) {
            ymax_loc = yValues[i];
        }
    }
    
    rango_x = (xmax_loc - xmin_loc) * 0.05;
    rango_y = (ymax_loc - ymin_loc) * 0.05;
    
    *xmin = xmin_loc - rango_x;
    *xmax = xmax_loc + rango_x;
    *ymin = ymin_loc - rango_y;
    *ymax = ymax_loc + rango_y;
}
