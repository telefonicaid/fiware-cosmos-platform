...
#define COMPAREVALUEFUNCTION "compareValueFunction="
      ...
      if (optional_compareKeyFunction.compare(0, strlen(COMPAREKEYFUNCTION), COMPAREKEYFUNCTION) == 0) {
        if ((type != "reduce") && (type != "parserOutReduce")) {
          fprintf(
            stderr,
            "samsonModuleParser: Error, compareKeyFunction available only on inputs to reduce or parserOutReduce operations, not to type:'%s'. Error at name:'%s', line:%d\n",
      ...
      if (optional_compareValueFunction.compare(0, strlen(COMPAREVALUEFUNCTION), COMPAREVALUEFUNCTION) == 0) {
        if ((type != "reduce") && (type != "parserOutReduce")) {
          fprintf(
            stderr,
            "samsonModuleParser: Error, compareValueFunction available only on inputs to reduce or parserOutReduce operations, not to type:'%s'. Error at name:'%s', line:%d\n",
            type.c_str(), name.c_str(), module_creator->items[pos - 1].line);
          exit(1);
        }
