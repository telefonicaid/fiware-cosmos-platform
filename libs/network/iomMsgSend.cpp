...
)
{
	...
	if ((dataLen != 0) && (data != NULL))
	{
		...
		LM_WRITES(to->name.c_str(), "data", ioVec[vecs].iov_base, ioVec[vecs].iov_len, LmfByte);
		++vecs;
	}
	...
	if ((packetP != NULL) && (packetP->message.ByteSize() != 0))
	{
		...
		LM_WRITES(to->name.c_str(), "google pbuffer", ioVec[vecs].iov_base, ioVec[vecs].iov_len, LmfByte);
		++vecs;
	}
	...
	if (packetP && (packetP->buffer != 0) )
	{
		...
		LM_WRITES(to->name.c_str(), "KV data", ioVec[vecs].iov_base, ioVec[vecs].iov_len, LmfByte);
		++vecs;
	}
	...
	s = writev(to->wFd, ioVec, vecs);
	if (s == -1)
	{
		LM_P(("writev(%s)", to->name.c_str()));
		to->msgsOutErrors += 1;
		return -1;
	}
	...
	if (s == 0)
	{
		LM_E(("writev(%s) returned ZERO bytes written", to->name.c_str()));
		to->msgsOutErrors += 1;
		return -1;
	}
	...
	to->msgsOut += 1;
	LM_T(LMT_MSG, ("written %d bytes to '%s' (fd %d)", s, to->name.c_str(), to->wFd));

	LM_WRITES(to->name.c_str(), "message header",  ioVec[0].iov_base, ioVec[0].iov_len, LmfByte);
	if (dataLen != 0)
		LM_WRITES(to->name.c_str(), "message data",  ioVec[1].iov_base, ioVec[1].iov_len, LmfByte);
