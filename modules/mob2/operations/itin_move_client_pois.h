			...
			{       
				continue;
                        ...
                        if(distance <= mob_conf_max_minutes_in_moves)
                        {
                                ...
                                struct tm time;
                                time.tm_mday = cur_loc.date.day.value;
                                time.tm_mon = cur_loc.date.month.value - 1;
                                time.tm_year = cur_loc.date.year.value + 100;
                                time.tm_hour = cur_loc.time.hour.value;
                                time.tm_min = cur_loc.time.minute.value;
                                time.tm_sec = cur_loc.time.seconds.value;
                                time_t raw_time = mktime(&time);
                                struct tm * timep = localtime(&raw_time);
                                cur_loc.date.week_day.value = timep->tm_wday;
