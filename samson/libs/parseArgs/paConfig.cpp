...
static void paConfigInit(void) {
  ...
  if (paPrefix)
    paPrefix              = strdup(paPrefix); if (paBuiltinPrefix)
    paBuiltinPrefix       = strdup(paBuiltinPrefix); if (paProgName)
    paProgName            = strdup(paProgName); if (paRcFileName)
    paRcFileName          = strdup(paRcFileName); if (paRcFileDir)
    paRcFileDir           = strdup(paRcFileDir); if (paGenericRcDir)
    paGenericRcDir        = strdup(paGenericRcDir); if (paVersionString)
    paVersionString       = strdup(paVersionString); if (paExecVersion)
    paExecVersion         = strdup(paExecVersion); if (paHelpFile)
    paHelpFile            = strdup(paHelpFile); if (paHelpText)
    paHelpText            = strdup(paHelpText); if (paTracelevels)
    paTracelevels         = strdup(paTracelevels); if (paManSynopsis)
    paManSynopsis         = strdup(paManSynopsis); if (paManShortDescription)
    paManShortDescription = strdup(paManShortDescription); if (paManDescription)
    paManDescription      = strdup(paManDescription); if (paManExitStatus)
    paManExitStatus       = strdup(paManExitStatus); if (paManAuthor)
...
int paConfig(const char *item, const void *value, const void *value2) {
  ...
  if (strcmp(item, "help file") == 0) {
    ...
    if (paPrefix != NULL)
      free(paPrefix); paPrefix = strdup((char *)val);
  } else if (strcmp(item, "builtin prefix") == 0) {
    if (paBuiltinPrefix != NULL)
      free(paBuiltinPrefix); paBuiltinPrefix = strdup((char *)val);
  } else if (strcmp(item, "prog name") == 0) {
    if (paProgName != NULL)
      free(paProgName); paProgName = strdup((char *)val);
  } else if (strcmp(item, "rc file") == 0) {
    if (paRcFileName != NULL)
      free(paRcFileName); paRcFileName = strdup((char *)val);
  } else if (strcmp(item, "rc dir") == 0) {
    if (paRcFileDir != NULL)
      free(paRcFileDir); paRcFileDir = strdup((char *)val);
  } else if (strcmp(item, "rc generic dir") == 0) {
    if (paGenericRcDir != NULL)
      free(paGenericRcDir); paGenericRcDir = strdup((char *)val);
  } else if (strcmp(item, "trace levels") == 0) {
    if (paTracelevels != NULL)
      free(paTracelevels); paTracelevels = strdup((char *)value);
    ...
    if (paExecVersion != NULL)
      free(paExecVersion); paExecVersion = strdup((char *)value);
    ...
    if (paLogFilePath != NULL)
      free(paLogFilePath); paLogFilePath = strdup((char *)value);
    ...
    if (paLogFileLineFormat != NULL)
      free(paLogFileLineFormat); paLogFileLineFormat = strdup((char *)value);
  } else if (strcmp(item, "log file time format") == 0) {
    if (paLogFileTimeFormat != NULL)
      free(paLogFileTimeFormat); paLogFileTimeFormat = strdup((char *)value);
  } else if (strcmp(item, "screen line format") == 0) {
    if (paLogScreenLineFormat != NULL)
      free(paLogScreenLineFormat); paLogScreenLineFormat = strdup((char *)value);
  } else if (strcmp(item, "screen time format") == 0) {
    if (paLogScreenTimeFormat != NULL)
      free(paLogScreenTimeFormat); paLogScreenTimeFormat = strdup((char *)value);
...
int paConfigActions(bool preTreat) {
  if (paVerbose5 == true)
    paVerbose4 = true; if (paVerbose4 == true)
    paVerbose3 = true; if (paVerbose3 == true)
    paVerbose2 = true; if (paVerbose2 == true)
    paVerbose = true; lmVerbose       = paVerbose;
