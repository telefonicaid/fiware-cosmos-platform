...
function usage() {
cat << EOF
Copies files to a remote host, optionally using a ssh tunnel.
...
if [[ -z "$pem" || -z "$user" || -z "$host" || -z "$from" || -z "$to" ]]; then
...
if [[ -z "$t_host" ]]; then
  copy "$pem" "$user" "$host" "22" "$from" "$to" "$exclude"
else
  t_port=$(unused_port "20200" "20400")
  tunnel "$t_pem" "$t_user" "$t_host" "$host" "22" "$t_port"
  copy "$pem" "$user" "127.0.0.1" "$t_port" "$from" "$to" "$exclude"
fi
