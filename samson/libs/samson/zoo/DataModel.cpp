...
void DataModel::PerformCommit(au::SharedPointer<gpb::Data> data, std::string command, int version,
  ...
  if (main_command == "add_stream_operation") {
    if (cmd.get_num_arguments() < 3) {
      ...
      LM_W(("Usage: add_stream_operation name operation -input \"input1 input2\" -output \"outputs1 outputs2 output3\""));
      error->set(
  ...
  if (main_command == "block") {
    ...
    for (size_t i = 0; i < items.size(); i++) {
      ...
      if (item->command() == "add") {
        LM_M(("Add block to queue:'%s' with version:%d", item->queue().c_str(), version));
