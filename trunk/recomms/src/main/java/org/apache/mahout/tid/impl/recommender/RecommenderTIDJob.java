...
import org.apache.mahout.tid.hadoop.item.AggrAndRecomObjReducer;
...
public final class RecommenderTIDJob extends AbstractJob {
	...
	public int run(String[] args) throws Exception {
		...
		if (mongoInput == true) {
			ReadJobConfig.readProperties(parsedArgs.get("--propertiesFilename")
					.get(0));
		...
		if (shouldRunNextPhase(parsedArgs, currentPhase)) {
			...
			Job aggregateAndRecommend = prepareJob(new Path(
					...
					AggrAndRecomObjReducer.class, VarLongWritable.class,
					RecommsWritable.class,
			...
			aggregateAndRecommendConf.set(
					AggrAndRecomObjReducer.ITEMID_INDEX_PATH, new Path(
							prepPath, PreparePreferenceMatrixJob.ITEMID_INDEX)
							.toString());

			aggregateAndRecommendConf.setInt(
					AggrAndRecomObjReducer.NUM_RECOMMENDATIONS,
					numRecommendations);
