...
namespace samson {
  namespace stream{
    ...
    void WorkerTask::fill( samson::gpb::CollectionRecord* record , const Visualization& visualization )
    {
      ...
      add( record , "operation" , stream_operation_->operation() , "left,different" );
...
#pragma mark
    ...
    void WorkerTask::generateKeyValues_reduce( samson::ProcessWriter *writer )
    {
      ...
      if( update_only )
        ...
        LM_T(LmtReduceOperation, ("Reduce %lu - %s begin" , get_id() , process_item_description().c_str() ));
      
      LM_T(LmtReduceOperation, ("Reduce %lu - %s KVRange %s" , get_id() , process_item_description().c_str() , range_.str().c_str() ));
