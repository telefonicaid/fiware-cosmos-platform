...
bool DataContainer::parse(AUTockenizer *module_creator,  int begin, int end) {
  ...
  while (pos < end) {
    if (module_creator->isSpecial(pos)) {
      fprintf(stderr, "samsonModuleParser: Error parsing data-type definition at line:%d",
    ...
    if (mainCommand == "optional") {
      if (firstFieldInData) {
        fprintf(stderr,
                "samsonModuleParser: Error, no optional field allowed if first in data structure (line:%d).\a\a\a\n",
                module_creator->items[pos].line);
        exit(1);
      } else {
        optionalData = true;
    ...
    if (mainCommand == "vector") {
      ...
      DataType data_type(_full_type, _name, DataType::container_vector, optionalData, valMask,
                         module_creator->items[pos].line);
      addItem(data_type);
      ...
      DataType data_type(_full_type, _name, DataType::container_list, optionalData, valMask,
                         module_creator->items[pos].line);
      addItem(data_type);
    } else {
      // Normal fields
      ...
      if (!module_creator->isSemiColom(pos)) {
        fprintf(stderr, "samsonModuleParser: Error parsing document at Data %s (%s %s), line:%d\n", name.c_str(),
                _full_type.c_str(), _name.c_str(), module_creator->items[pos].line);
        exit(1);
      }
      pos++;

      DataType data_type(_full_type, _name, DataType::container_none, optionalData, valMask,
                         module_creator->items[pos].line);
      addItem(data_type);
    }
  ...
  if (pos != (end + 1)) {
    fprintf(
      stderr,
      "samsonModuleParser: Error parsing data-type definition. Number of items is not correct at the end of parsing, ends at line:%d",
      module_creator->items[pos].line);
    exit(1);
  }
