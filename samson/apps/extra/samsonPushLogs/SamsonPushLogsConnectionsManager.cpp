...
void SamsonPushLogsConnection::Run()
{
    ...
    while( true )
    {
        ...
        while (timestamp > first_timestamp + ntimes_real_time_ * cronometer.seconds())
        {
            ...
            {
                LM_M(("Sleeping %s with timestamp:%s, elapsed_time:%lf(%d%% realtime) since %s", dataset_->GetQueueName(),  time_read_str, ntimes_real_time_ * cronometer.seconds(), int(100*ntimes_real_time_), time_init_str));
            }

            if ((count_sleeps%10000 == 0) && (timestamp > (first_timestamp + ntimes_real_time_ * cronometer.seconds() + 900)))
            {
                LM_M(("An inactivity long gap for %s with timestamp:%s, elapsed_time:%lf(%d%% realtime) since %s", dataset_->GetQueueName(),  time_read_str, ntimes_real_time_ * cronometer.seconds(), int(100*ntimes_real_time_), time_init_str));
            }
        ...
        {
            LM_M(("Pushing with time:%s (first:%s, elapsed:%lf(%d%% real_time)) to SAMSON from dataset:%s", time_read_str, time_init_str, ntimes_real_time_ * cronometer.seconds(), int(100*ntimes_real_time_), dataset_->GetQueueName()));
        }
