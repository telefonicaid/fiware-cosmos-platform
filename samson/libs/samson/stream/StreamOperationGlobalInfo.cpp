    ...
    void StreamOperationGlobalInfo::Review(gpb::Data *data ) {
      ...
      while ( stream_operations_range_info_.size() > 0 )
      {
        ...
        if( info->range_division_necessary() )
        {
          ...
          KVRange range1( range.hg_begin , range.hg_begin + range_size/2 );
          KVRange range2( range.hg_begin + range_size/2 , range.hg_end );
          ...
          delete info; // Remove origina info structure
