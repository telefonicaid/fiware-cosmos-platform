...
namespace samson{ namespace system{
    ...
    public:
        ...
        void update_accumulator( Value* key , Value * state , Value ** values , size_t num_values , samson::KVWriter *writer )
        {
            ...
            emit_log( "debug" , au::str("Processing top.accumulateion state %s - %s with %lu values" , key->str().c_str() , state->str().c_str() , num_values ) , writer );
        ...
        void update_category_with_value( Value * state  , Value* new_value )
        {
            ...
            for ( size_t p = 0 ; p < state->get_vector_size() ; p++ )
            {
                ...
                if( strcmp( vector_value_concept , new_value_concept ) == 0 )
                {
                    ...
                    while( (p > 0) && ( state->get_value_from_vector(p)->get_double_from_map("total",0) >  state->get_value_from_vector(p-1)->get_double_from_map("total",0) ) )
                    {
                    ...
                    while( (p < (state->get_vector_size()-1)) && ( state->get_value_from_vector(p)->get_double_from_map("total",0) < state->get_value_from_vector(p+1)->get_double_from_map("total",0) ) )
                    {
                        // Swap p and p-1 elements
                        state->swap_vector_components( p , p+1 );
                        p++;
        ...
        {
            emit_log( "debug" , au::str("Processing top.category state %s - %s with %lu values" , key->str().c_str() , state->str().c_str() , num_values ) , writer );
            ...
            for ( size_t i = 0 ; i < num_values ; i++ )
            {
                emit_log("debug", au::str("Updating state %s with new value %s" , state->str().c_str() , values[i]->str().c_str() ), writer );
